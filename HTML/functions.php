...
{   $id = $database->prepare('SELECT memberID FROM members WHERE username= :username AND active="Yes"');
    ...
    $row = $id->fetch();
    $a=$row['memberID'];
    $b = $database->prepare('INSERT INTO shipping (id,firstName,lastName,address,town,county,postcode) VALUES (:id,:firstName,:lastName,:address,:town,:county,:postcode)');
    $b->execute(array(':id' => $a,
                    ...
                    ':county'=>$_POST['shipping_state'],
                    ':postcode'=>$_POST['shipping_postcode']));
                    
}

 function checkBillingAddress($database,$username)
{
    return false;
}
